...
def detection_head(inputs, dim=256):
    ...
    x = tf.keras.layers.Dense(int(dim / 2), activation="relu")(
        x
    )  # Fully connected layer
    x = tf.keras.layers.Dropout(0.5)(x)  # Regularization
    x = tf.keras.layers.Dense(int(dim / 4), activation="relu")(
        x
    )  # Fully connected layer
    x = tf.keras.layers.Dropout(0.5)(x)  # Regularization
...
def heatmap_head(inputs, h, w):
    ...
    x = tf.keras.layers.Conv2D(1, 1, padding="same", activation="sigmoid")(
        x
    )  # Output: heatmap
    x = tf.keras.layers.Resizing(h, w, interpolation="bilinear")(
        x
...
def pretrained_backbone(
    inp, unfreeze_frac=0.3, h=None, w=None, c=None, trainable=False
):
    ...
    x = backbone(
        x, training=False
    )  # Extract features without updating backbone weights
...
def generator(h, w, c, unfreeze_frac=0.3, trainable=False, use_heatmap=False):
    ...
    features = pretrained_backbone(
        inp, h=h, w=w, c=c, unfreeze_frac=unfreeze_frac, trainable=trainable
    )
