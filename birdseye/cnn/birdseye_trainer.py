...
class BirdsEyeTrainer:
    ...
    def build_file_lists(self):
        ...
        self.train_files = collect_paths(
            self.config["train_dates"], self.config["train_dirlists"]
        )
        self.val_files = collect_paths(
            self.config["val_dates"], self.config["val_dirlists"]
        )
        ...
        if self.config.get("use_heatmaps", False):
            ...
            metrics = [
                tf.keras.metrics.BinaryCrossentropy(
                    from_logits=self.config.get("logits", False), name="bce"
                ),
            ...
            metrics = [
                tf.keras.metrics.BinaryCrossentropy(
                    from_logits=self.config.get("logits", True), name="bce"
                ),
                tf.keras.metrics.BinaryAccuracy(
                    threshold=self.config.get("thresh", 0.5), name="bin_acc"
                ),
                tf.keras.metrics.F1Score(
                    threshold=self.config.get("thresh", 0.5), name="f1"
                ),
    ...
    def train(self):
        ...
        callbacks = [
            ...
            tf.keras.callbacks.ReduceLROnPlateau(
                monitor=self.config["monitor"], factor=0.5, patience=3, min_lr=0
            ),
            tf.keras.callbacks.EarlyStopping(
                monitor=self.config["monitor"], patience=15
            ),
...
if __name__ == "__main__":
    ...
    val = str(
        len(glob2.glob(os.path.join(models_dir, filename_prefix + "*.weights.h5"))) + 1
    ).rjust(3, "0")
