...
class TileLoader:
    ...
    def tile_image_and_label(self, image_np, labels):
        ...
        # tile_rows = math.ceil((h-2*buffer)/th)
        # tile_cols = math.ceil((w-2*buffer)/tw)
        # stride_y = math.ceil(th - (h-2*buffer)/tile_rows)
        # stride_x = math.ceil(tw - (w-2*buffer)/tile_cols)
