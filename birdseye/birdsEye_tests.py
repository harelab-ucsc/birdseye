...
# generate several targets
# generate several nonoverlapping targets
...
def targetGenerate(img, pos, tgt_radius, test_object):
    tgt = shift_point(pos, (XRES,YRES), 2, 2)
    # cv2.circle(img, tgt, tgt_radius, 1, -1)
...
def offFrameCheck_Test(test_object, tgts):
    ...
    ret, reproj = test_object._2DFrameCheck(np.eye(4), \
                                    np.eye(4), \
                                    np.array([[1 if (i == j or i == 3 ) else 0 for i in range(4)]for j in range(4)]), \
...
if __name__ == '__main__':
    dir_path = '/home/mwmaster/catch/'
    save_name = os.path.join(dir_path, 'data')
    db_name = 'flight_data'
    dbc = dbConnector(db_name)
    tst = birdsEye(dbc, res=(XRES, YRES), K=K, db_name=db_name, img_dir=dir_path, save_name=save_name)
