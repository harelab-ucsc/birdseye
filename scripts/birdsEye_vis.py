...
for i, data in enumerate([data_raw, data_rect]):
    ...
    bproj = np.array(
        data["bproj"]
    )  # back-projection of clicks (to and from pixel space)
    ...
    print(
        "AprilTag 3D-projection accuracy (meters): \n",
        bproj.mean(axis=0),
        "+/- ",
        bproj.std(axis=0),
    )
    print(
        "Click projection accuracy (pixels): \n",
        reproj.mean(axis=0),
        "+/- ",
        reproj.std(axis=0),
    )
    ...
    ax[0].scatter(
        bproj[:, 0], bproj[:, 1], c=colors[0], alpha=0.1, label=tmp + "BackProj"
    )  # bproj[:,2],
    ax[0].scatter(
        bproj[:, 0].mean(),
        bproj[:, 1].mean(),
        c=colors[2],
        marker=marker,
        label=tmp + "Mean Error",
    )  # bproj[:,2].mean,
    ax[1].scatter(
        reproj[:, 0], reproj[:, 1], c=colors[1], alpha=0.1, label=tmp + "Reproj"
    )
    ...
    lbl = (
        tmp
        + f"BackProj Norm: {np.linalg.norm(bproj, axis=1).mean():.3f} +/- {np.linalg.norm(bproj, axis=1).std():.3f} m"
    )
    ax2[0].hist(
        np.linalg.norm(bproj, axis=1),
        bins=100,
        density=True,
        color=colors[0],
        label=lbl,
    ...
    lbl = (
        tmp
        + f"ReProj Norm: {np.linalg.norm(reproj, axis=1).mean():.1f} +/- {np.linalg.norm(reproj, axis=1).std():.1f} px"
    )
    ax2[1].hist(
        np.linalg.norm(reproj, axis=1),
        bins=100,
        density=True,
        color=colors[1],
        label=lbl,
