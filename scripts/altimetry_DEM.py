...
class BagProcessor:
    def __init__(self, input_bags, ds_dir, radalt_topic, ins_topic, ds_rate=200):
        ...
        self.kernel = C(1.0) * RationalQuadratic(
            length_scale=1.0,
            alpha=2e-06,
            length_scale_bounds=(1e-5, 1e5),
            alpha_bounds=(1e-15, 1e5),
        )  # + WhiteKernel(noise_level=1e-2, noise_level_bounds=(1e-18, 1e5))
        ...
        self.GP = GaussianProcessRegressor(
            ...
            optimizer=lambda obj_func, initial_theta, bounds: scipy.optimize.minimize(
                obj_func,
                initial_theta,
                method="L-BFGS-B",
                bounds=bounds,
    ...
    def match_pairs(self, radalt_msgs, ins_msgs):
        ...
        tmp = [[rad[i[1]], ins[i[0]]] for i in tmp]
        print(
            f"    time mismatch across {len(rad)} radalt messages is {sum([i[0] - i[1] for i in tmp]) / 1e9:.04f}s"
        )
        return pairs
    ...
    def GP_train(self):
        ...
        print("  training GP...")
        print(
            f"    initial performance: {self.GP.score(x_tmp, y_tmp)}, {self.GP.kernel}"
        )
        start = time.time()
        self.GP.fit(x_tmp, y_tmp)
        print(
            f"    optimized performance: {self.GP.score(x_tmp, y_tmp)}, {self.GP.kernel_}"
        )
    ...
    def plot_dem_points(self, title="DEM", elev=30, azim=30):
        ...
        self.ax.set_xlim(
            self.center[0] - self.halflength, self.center[0] + self.halflength
        )
        self.ax.set_ylabel("North (m)")
        self.ax.set_ylim(
            self.center[1] - self.halflength, self.center[1] + self.halflength
        )
        self.ax.set_zlabel("Z (m)")
        self.ax.set_zlim(
            self.center[2] - self.halflength, self.center[2] + self.halflength
        )
    ...
    def plot_GP_surface(self):
        ...
        for i in range(self.boundary):
            for j in range(self.boundary):
                print(
                    f"  progress: ({i + 1}, {j + 1}) of ({self.boundary}, {self.boundary})",
                    end="\r",
                )
        ...
        dst = np.squeeze(Y)
        scatter = self.ax.scatter(
            src[:, 0], src[:, 1], dst, c=dst, cmap="terrain", marker="o", s=10
        )
    ...
    def process_bag(self):
        ...
        try:
            storage_options = StorageOptions(
                uri=self.input_bags[self.bag_index], storage_id="mcap"
            )
            ...
            storage_options = StorageOptions(
                uri=self.input_bags[self.bag_index], storage_id="sqlite3"
            )
            converter_options = ConverterOptions(
                input_serialization_format="cdr", output_serialization_format="cdr"
            )
        ...
        print(
            f"reading bag {self.input_bags[self.bag_index]} at {self.ds_rate} downsampling rate"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Fix image timestamps in a ROS2 bag file using INS messages."
    )
    ...
    parser.add_argument(
        "--radalt_topic", help="Radar Altimeter topic name (e.g., /radalt/data)"
    )
    ...
    processor = BagProcessor(
        args.input_bags, args.ds_dir, args.radalt_topic, args.ins_topic
    )
